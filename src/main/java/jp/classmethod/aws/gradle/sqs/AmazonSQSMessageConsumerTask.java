...
public class AmazonSQSMessageConsumerTask extends AbstractAmazonSQSTask {
    ...
    public void consumeMessages() {
        ...
        while (counter < maxNumberOfMessages) {
            ReceiveMessageRequest request = new ReceiveMessageRequest().withQueueUrl(queueUrl).withMaxNumberOfMessages(MAX_MESSAGE_CONSUME_BATCH_SIZE).withVisibilityTimeout(30);
            List<DeleteMessageBatchRequestEntry> messagesToDelete = sqs.receiveMessage(request).getMessages().stream().map(message -> {
                if (showMessages) {
                    getLogger().lifecycle(String.format("Read message id: %s, message body: %200s", message.getMessageId(), message.getBody()));
                }
